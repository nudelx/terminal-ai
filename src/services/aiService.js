...
export const sendMessage = async (client, history, modelId, provider = "openrouter") => {
  try {
    ...
    const errorHandlers = {
      401: () => console.error(chalk.red(`Invalid API key. Please check your ${provider === "gemini" ? "GEMINI_API_KEY" : "OPENROUTER_API_KEY"}.`)),
      404: () => console.error(chalk.red(`Model not found: ${getModelByKey(modelId, provider)?.id}.`)),
